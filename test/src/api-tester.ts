...
export class ApiTester {
  ...
  test<TSuccess, TFail>(
    fn: () => Promise<AxiosResponse<TSuccess>> | FailType<TFail>,
    desc: string
  ) {
    class StatusSetter {
      constructor(private logger: Logger, private desc: string) {}
      status<
        TStatus extends SuccessCode | number,
        TResponse = TStatus extends SuccessCode ? TSuccess : TFail
      >(code: TStatus) {
    ...
    class ResponseSetter<TResponse> {
      ...
      private ref: TResponse | ((x: TResponse) => boolean) | symbol =
        this.empty;
      ...
      async toPromise(): Promise<ApiTestResult<TResponse>> {
        ...
        if (response.status === this.status) {
          ...
          this.logger.error(
            `FAIL ${this.desc}. The status code mismatch, expected '${this.status}' but given '${response.status}'`
          );
        }
