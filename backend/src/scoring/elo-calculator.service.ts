...
export class EloCalculatorService {
  ...
  newRating(rating: number[]) {
    ...
    return rating.map((x, i) => x + scaleFactor * (scores[i] - expectedScores[i]));
  }

  scoreAmplifierDistribution(participants: number) {
    return Array.from({ length: participants }, (x, i) => this.distrFn(i + 1, participants));
  }

  expectedScores(rating: number[]) {
    return Array.from(
      {  length: rating.length },
      (x, m) => this.expectationForPlace(m, rating));
  }
  ...
  private expectationForPlace(place: number, rating: number[]) {
    ...
    return Array.from(
      { length: rating.length }, 
      (x, n) => place === n ? 0 : this.expectedScore(rating[place], rating[n]))
