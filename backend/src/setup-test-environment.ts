...
export async function setupTestEnvironment(): Promise<void> { 
  ...
  if (certFilename) {
    try {
      ...
      const verifySignedJwtWithCertsAsync = OAuth2Client.prototype.verifySignedJwtWithCertsAsync;
      OAuth2Client.prototype.verifySignedJwtWithCertsAsync = async (jwt: string, certs: Certificates | PublicKeys, requiredAudience?: string | string[], issuers?: string[], maxExpiry?: number) => {
        return verifySignedJwtWithCertsAsync(jwt, { ...certs, [kid]: cert }, requiredAudience, issuers, maxExpiry);
      };
    } catch(e) {
